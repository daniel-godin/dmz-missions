        ...
        for (let j = 0; j < arrayFirstLevel.length && j < 20; j++) { // Second Loop (NESTED IN FIRST):
            ...
            for (let k = 0; k < arraySecondLevel.length && k < 20; k++) { // THIRD LOOP (NESTED IN 2ND LOOP).  Loops through each Mission Object.
                ...
                let complete = missionDataObject.complete; // Two below if (statements) change the DOM Mission Checkbox to be "checked" or "not checked".
                    if (complete == true) { missionTitleStatus = 'mission-complete'; missionCheckbox = "checked"; };
                    if (complete == false) { missionTitleStatus = ''; missionCheckbox = '' };
                ...
                let missionTitleMinimizeStatus;
                let storageKey = localStorage.getItem(`${missionId}`) // This is using a mission specific identifier of "missionId" as the key in localStorage.  Then grabbing the value of that key, using that value to set whether a title should be minimized or not.

                if (storageKey == 'hideBox') { missionTitleMinimizeStatus = 'hide' }
                if (storageKey == 'showBox' || 'null') { missionTitleMinimizeStatus = '' }
                ...
                DOMAttachmentPoint.insertAdjacentHTML('beforeend', `
                    <div class='fob-mission-container ${missionUnlockedStatus} ${missionTitleStatus}'>
                        <div class='fob-mission-title-container'>
                            ...
                            <div class='fob-mission-complete-container ${userStatus}'>
                                <input type="checkbox" class='fob-mission-checkbox' data-fob-mission-id='${missionId}' data-mission-dot-notation="${missionDotNotation}" ${missionCheckbox} />
                            </div>
                        </div>
